...
function getTotalCount() {
  ...
  .then(response => {
    ...
    // == 비동기 요청 성공 시
    // if(respose.status === 200) // 아래 if문과 같은 의미
    if(response.ok){
...
function getCompleteCount(){
  ...
  */
  ...
  fetch("/todo/completeCount") // 비동기 요청해서 결과 데이터 응답
  .then(response => {
...
deleteBtn.addEventListener("click", () => {
  ...
  .then(response => {
    if(response.ok) return response.text();
    throw new Error("삭제 여부 변경 실패 : " + response.status)
  })
  .then(result => {
    // 팝업 닫기
    document.querySelector("#popupLayer").classList.add("popup-hidden");
...
updateBtn.addEventListener("click", () => {
  ...
  // 서버로 제출 도어야 하는 값을 JS 객체로 형태로 묶기
  const obj = {}; // 빈 객체 생성
