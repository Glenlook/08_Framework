...
memberNickname?.addEventListener("input", () => {
  // input이벤트 : 입력과 관련된 모든 동작 (JS를 이용한 값 세팅 제외)
  ...
  .then(result => {
    // result == 첫 번쨰 then 에서 return 된 값
...
if(imageInput != null) { // 프로필 변경 화면인 경우
  ...
  imageInput.addEventListener("change", e => {
    ...
    if(file === undefined) {
      ...
      /* 이전 선택한 파일 유지하는 코드 */
      // -> 이전 선택한 파일을 저장할 전역 변수(lastValildFile) 선언
